...
try {
    db.serialize(function () { // serialize means execute one statement at a time
        ...
        db.run("CREATE TABLE RESTAURANTS (id INT, name TEXT, imagelink TEXT, menuId INT)");
        db.run("CREATE TABLE MENUS (id INT, title TEXT, menuItem INT)");
        db.run("CREATE TABLE MENU_ITEMS (id INT, name TEXT, price REAL)");
